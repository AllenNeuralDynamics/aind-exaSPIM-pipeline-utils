...
def get_auto_parameters(args: Dict) -> Dict:  # pragma: no cover
    ...
    macro_iterative_solver = f"{results_path}/macro_iterative_solver.ijm"
    ...
    return {
        ...
        "macro_iterative_solver": macro_iterative_solver,
    }
...
def main():  # pragma: no cover
    ...
    if args["do_detection"]:
        ...
        if args["do_registrations"]:
            ...
            logger.info("Copying %s -> %s", ip_src, results_path)
            shutil.copytree(
                ip_src,
                os.path.join(results_path, "interestpoints.n5"),
                dirs_exist_ok=True
            )
